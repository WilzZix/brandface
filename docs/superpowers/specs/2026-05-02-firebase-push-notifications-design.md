...
| `lib/core/di/app_di.config.dart` | Auto-regenerated by `build_runner` — picks up new `@LazySingleton` annotations |
...
Both classes registered as `@LazySingleton` via `injectable` (matches existing pattern, e.g. `AppLanguageService`). DI graph regenerated with `dart run build_runner build --delete-conflicting-outputs`.
